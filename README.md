...
The active controller reads the downsampled rendered image for horizontal and vertical steering; game coordinates are retained for collision and reward bookkeeping. Two independent categorical policies use clipped on-policy updates, separate value baselines, axis-specific progress rewards, adaptive exploration, elite ring memories, and a regression checkpoint. Policy temperature and update strength anneal as training matures so late runs increasingly execute learned actions instead of remaining artificially noisy. A separate binary front-limb policy learns firing from retinal target alignment, direct hit/miss credit, and successful-shot memories. Its rewards do not update the ring policies. A small learned target-alignment movement is enabled only when the target is visible in a safe gap before the next ring, limiting interference with the established steering behavior.

Stationary mode repeats one ten-ring course for controlled learning tests. Randomized mode changes the layout between episodes to test whether the retinal steering behavior transfers. The retinal salience channels, action-selection circuit, neural-location overlay, and flight animation remain educational approximations rather than validated predictions from the full fly connectome.
...
The replay includes `Skip 5`, `Skip 10`, `Skip 25`, `Skip 50`, and `Skip 100` controls. These simulate complete training episodes asynchronously, preserve the learned policy, and leave the next episode ready for visual inspection. The graph shows individual results and 10-episode moving averages, with range and series controls for inspection.
